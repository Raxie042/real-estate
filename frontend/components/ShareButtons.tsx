...
export default function ShareButtons({ listingId, title, description, imageUrl }: ShareButtonsProps) {
  const [showMenu, setShowMenu] = useState(false);
  const [copied, setCopied] = useState(false);

  const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';
  const listingUrl = `${baseUrl}/properties/${listingId}`;
  ...
  const shareLinks = {
    ...
    instagram: listingUrl, // Instagram requires manual sharing
  ...
  const handleShare = async (platform: keyof typeof shareLinks) => {
    if (platform === 'email') {
      ...
      // Instagram doesn't have web sharing, so copy link
      try {
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center space-x-2 px-4 py-2 border border-[#C9A96A] text-[#C9A96A] rounded-lg hover:bg-[#C9A96A] hover:text-white transition"
      >
